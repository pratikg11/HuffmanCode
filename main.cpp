...
vector<node> nodeArray;// Use to store all the nodes that may be created in the whole process
node root;
...
void compressionAchieved(string str,string encode,int count)
{
	int i=1,bits;
	bool temp=true;
	while(temp)
	...
	int n=str.size()*bits;
	int m = encode.size();
	cout<<"Size before compression : "<<n<<" bits"<<endl;
	cout<<"Size after compression : "<<m<<" bits"<<endl;
	float result=(float(n-m)/n)*100;
	cout<<"Compression achieved : "<<result<<"%"<<endl;
}
...
void getHuffmanCode()
{
 ...
 decode(encode);
 compressionAchieved(name1,encode,characters.size());
}
